...
#     @tf.function
#     def get_tensor_product_displacement(single_mode_disp, mode):
#         '''
...
#         if mode == 1: 
#             #other mode is 2 
#             identity = qt.identity(self.parameters["N_cav2"])
#         elif mode == 2:
#             identity = qt.identity(self.parameters["N_cav1"])
#         else: 
...
#                 block_list_full.append(multi_list_full)

#             ds_full.append(block_list)
...
#         ds_end = get_tensor_product_displacement(self.batch_construct_displacement_operators(D), mode)
#         ds_g = get_tensor_product_displacement(self.batch_construct_displacement_operators(Bs), mode)  
...
#         return blocks
    ...
    def batch_construct_block_operators(
        ...
        #---------------------------------------------------------------------------------
        # phi = phi - pi/2
        Phis1 = phis1 - tf.constant(np.pi, dtype=tf.float32) / tf.constant(
            2, dtype=tf.float32
        )
        #theta = theta/2
        Thetas1 = thetas1 / tf.constant(2, dtype=tf.float32)
        
        #Reshaping these lists of angles for some reason
        Phis1 = tf.cast(
            tf.reshape(Phis1, [Phis1.shape[0], Phis1.shape[1], 1, 1]), dtype=tf.complex64
        )
#         etas = tf.cast(
#             tf.reshape(etas, [etas.shape[0], etas.shape[1], 1, 1]), dtype=tf.complex64
#         )
        Thetas1 = tf.cast(
            tf.reshape(Thetas1, [Thetas1.shape[0], Thetas1.shape[1], 1, 1]),
            dtype=tf.complex64,
        )
        #------------------------------second set of angles
        # phi = phi - pi/2
        Phis2 = phis2 - tf.constant(np.pi, dtype=tf.float32) / tf.constant(
            2, dtype=tf.float32
        )
        #theta = theta/2
        Thetas2 = thetas2 / tf.constant(2, dtype=tf.float32)
        
        #Reshaping these lists of angles for some reason
        Phis2 = tf.cast(
            tf.reshape(Phis2, [Phis1.shape[0], Phis2.shape[1], 1, 1]), dtype=tf.complex64
        )
        Thetas2 = tf.cast(
            tf.reshape(Thetas2, [Thetas2.shape[0], Thetas2.shape[1], 1, 1]),
            dtype=tf.complex64,
        )
        #------------------------------------------------------------------------------
        ...
        #e^iphi
        #------- First
        exp1 = tf.math.exp(tf.constant(1j, dtype=tf.complex64) * Phis1)
        im = tf.constant(1j, dtype=tf.complex64)
        exp1_dag = tf.linalg.adjoint(exp1)
        cos1 = tf.math.cos(Thetas1)
        sin1 = tf.math.sin(Thetas1)
        #-------- Second
        exp2 = tf.math.exp(tf.constant(1j, dtype=tf.complex64) * Phis2)
        im = tf.constant(1j, dtype=tf.complex64)
        exp2_dag = tf.linalg.adjoint(exp2)
        cos2 = tf.math.cos(Thetas2)
        sin2 = tf.math.sin(Thetas2)
...
#         lr = (cos - im * sin * cos_e) * ds_e
        ll =  ((im * im * cos2 * cos1) + (exp2_dag * sin2 * exp1 * sin1)) * ds_g
        ul = im* ((exp2 * sin2*cos1) + (cos2 * exp1 * sin1)) * ds_e
        lr =  im* ((exp2_dag * sin2*cos1) + (cos2 * exp1_dag * sin1)) * ds_g
        ur =((exp2 * sin2 * exp1_dag * sin1) + (im * im * cos2 * cos2)) * ds_e
        # without pi pulse, block matrix is:
        # (ul, ur)
        # (ll, lr)
        # however, with pi pulse included:
            ...
            for epoch in range(self.parameters["epochs"] + 1)[1:]:
                for _ in range(self.parameters["epoch_size"]):
                    with tf.GradientTape() as tape:
                        ...
                        phis1 = entry_stop_gradients(self.phis1, self.phi_mask)
                        phis2 = entry_stop_gradients(self.phis2, self.phi_mask)
                        if self.parameters["use_etas"]:
                            etas = entry_stop_gradients(self.etas, self.eta_mask)
                        else:
                            etas = self.etas
                        thetas1 = entry_stop_gradients(self.thetas1, self.theta_mask)
                        thetas2 = entry_stop_gradients(self.thetas2, self.theta_mask)

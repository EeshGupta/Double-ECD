...
#         self.__init__(**parameters)
    ...
    def batch_construct_block_operators(
        ...
        ll = tf.constant(1j, dtype=tf.complex64)* cos * ds_g
        ul = exp * sin * ds_e
        lr = tf.constant(-1, dtype=tf.complex64) * exp_dag * sin * ds_g
        ur = tf.constant(1j, dtype=tf.complex64)*cos * ds_e

        # EG: Ignore these comments below by Alec. In orginal version, the matrices below should be swapped anyway
        ...
        #EG: Since two ECD pulses with pi pulse in each, the final form should not include pi pulse at all 
        blocks = tf.concat(
            [
